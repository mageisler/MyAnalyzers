...
void analyzePVMC()
{
  ...
  files["MC-QCD"]   = TFile::Open("/user/geisler/CMSSW/CMSSW_5_3_6_patch1/src/MGeisler/PVStudy/test/VertexAnalyzer_Resolution_MC-QCD_histos.root");
...
void FillHistos(string type, vector<string> sampleNames, vector<string> pvNames_tmp,
		double ***arrNtrks, double ***arrErrNtrks,
		double ****arrType, double ****arrErrType) { // type = "Res" or "Pull"

  int nEntries_tmp = nTrkMax_ - nTrkMin_ + 1;
  int nFiles = files.size();
  int nPVCol = pvNames_tmp.size();
  ...
  for (vector<string>::iterator iname = sampleNames.begin(); iname != sampleNames.end(); iname++, fid++) {
    int pvid = 0;
    for (vector<string>::iterator jname = pvNames_tmp.begin(); jname != pvNames_tmp.end(); jname++, pvid++) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    //tex2_[i]->Draw();
    canvas_[i]->Write();
    canvas_[i]->Print(TString("../pictures/"+type+coord[i]+".png"));
    canvas_[i]->Print(TString("../pdfs/"+type+coord[i]+".pdf"));
  }
}
