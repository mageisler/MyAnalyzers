...
from AnalysisTools import GetListOfFilesWithEnding, GetListOfSubdirectories, GetMergedProf3DRek, npu_ranges, pt_rangesTRACK, eta_rangesTRACK,  Create1DProf, npu_namesP, pt_namesPTRACK, eta_namesPTRACK, DrawProfs2DWithHisto, colorsMap, my_colorsP, GetProjection, UpperEdges, LowerEdges, CreateFractionProfsRek
...
letters = 'p:k:c:t:'
keywords = ['path','keyword','collections','tag']
...
output_name = 'TrackValidator_' + label
...
out = ROOT.TFile(output_label+".root","RECREATE")

for ass_ite in range(len(prof_npu[0])):
	prof_eta_final[0][0][0][ass_ite].Write("eta_"+collections[ass_ite].split("/")[1])
	prof_pt_final[0][0][0][ass_ite].Write("pt_"+collections[ass_ite].split("/")[1])
	prof_npu_final[0][0][0][ass_ite].Write("npu_"+collections[ass_ite].split("/")[1])
...
for axis1_ite in range(len(prof_eta_final[0])):
    ...
    for axis2_ite in range(len(prof_eta_final[0][0])):
		...
		savename = 'eta/' + output_label + '/' + output_name + 'VsEta_Npu' + npu_namesP[axis1_ite] + '_Pt' +pt_namesPTRACK[axis2_ite]
...
for axis1_ite in range(len(prof_pt_final[0])):
	...
	histo_help_pt.SetTitle(histo_title)
        ...
        savename = 'pt/' + output_label + '/' + output_name + 'VsPt_Npu' + npu_namesP[axis1_ite] + '_Eta' +eta_namesPTRACK[axis2_ite]
...
for axis1_ite in range(len(prof_npu_final[0])):
	...
	histo_help_npu.SetTitle(histo_title)
        ...
        savename = 'npu/' + output_label + '/' + output_name + 'VsNpu_Eta' + eta_namesPTRACK[axis1_ite] + '_Pt' + pt_namesPTRACK[axis2_ite]
