...
for sec_ite in range(len(h3[0])):
	...
	print cleans[sec_ite] , ": Purity:", pur, "  - Efficiency:", eff, "  - Product:" , pur*eff
...
outputname = "UnCleanedComparison_" + secondaries + "_"
...
for i in range(len(h1_sum)):
	...
	if "Clean" in h1_sum[i].GetName():
		h1_sum[i].SetName("Filtered")

	if "Unclean" in h1_sum[i].GetName():
		h1_sum[i].SetName("Unfiltered")
...
if "Val" in xaxis:
	...
	h1_sum[0].GetXaxis().SetTitle("#sigma_{d_{xy}}")

#DrawTDRStyle(histos=h1_sum, profiles=(), additional=(), logs=(1,0,0), legend=2, scale=False, zeroSuppression=False, changeLineStyle=False, drawOption='p', name=h1_name, moreTickz=False, usePalette='default')
...
for i in range(len(h_pur)):
	...
	if "Clean" in h_pur[i].GetName():
		h_pur[i].SetName("Filtered")
	if "Clean" in h_eff[i].GetName():
		h_eff[i].SetName("Filtered")
	
	if "Unclean" in h_pur[i].GetName():
		h_pur[i].SetName("Unfiltered")
	if "Unclean" in h_eff[i].GetName():
		h_eff[i].SetName("Unfiltered")
